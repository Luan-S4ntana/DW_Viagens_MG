...
def cidade():
    ...
    mapeamento_pais_estado = estados[['id_pais', 'id_estado']].drop_duplicates().set_index('id_pais')

    for index, row in cidades_fato.iterrows():
        if row['id_estado'] == 1:
            id_pais_atual = row['id_pais']
            novo_id_estado = mapeamento_pais_estado.loc[id_pais_atual, 'id_estado']
            cidades_fato.at[index, 'id_estado'] = novo_id_estado
    ...
    cidades = cidades.drop(['id_pais'],axis=1)
